...
func (w *VictoriaMetricsWriter) send(timeSeries []prompb.TimeSeries) error {
	...
	req, err := http.NewRequestWithContext(w.ctx, "POST", w.url, bytes.NewReader(compressed))
	if err != nil {
	...
	log.Printf("Successfully pushed %d metrics to VictoriaMetrics", len(timeSeries))
	return nil
}
