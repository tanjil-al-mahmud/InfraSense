...
import (
	...
	"github.com/infrasense/snmp-collector/internal/collector"
	"github.com/infrasense/snmp-collector/internal/config"
	"github.com/infrasense/snmp-collector/internal/metrics"
	_ "github.com/lib/pq"
)
...
func main() {
	...
	logger.Info("starting snmp collector", "event", "collector_start", "collector_type", "snmp")
	...
	snmpCollector := collector.NewSNMPCollector(
		db,
		metricsWriter,
	...
	// Start collector
	if err := snmpCollector.Start(); err != nil {
		logger.Error("failed to start snmp collector", "event", "collector_start_error", "error", err.Error())
		os.Exit(1)
	}

	// Start health check server
	http.HandleFunc("/health", func(w http.ResponseWriter, r *http.Request) {
		deviceCount := snmpCollector.GetDeviceCount()
	...
	http.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		fmt.Fprintf(w, "# HELP snmp_collector_devices Number of devices being monitored\n")
		fmt.Fprintf(w, "# TYPE snmp_collector_devices gauge\n")
		fmt.Fprintf(w, "snmp_collector_devices %d\n", snmpCollector.GetDeviceCount())
	})
	...
	logger.Info("shutting down snmp collector", "event", "collector_shutdown")
	snmpCollector.Stop()
	...
	logger.Info("snmp collector shutdown complete", "event", "collector_shutdown_complete")
}
